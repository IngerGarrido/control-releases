...
try {
    ...
    if ($method === 'GET') {
        $rows = $db->query("SELECT * FROM etiquetas ORDER BY orden ASC, id ASC")->fetchAll(PDO::FETCH_ASSOC);
    ...
    if ($method === 'POST') {
        ...
        $ord = (int)$db->query("SELECT COALESCE(MAX(orden),-1)+1 FROM etiquetas")->fetchColumn();
        $stmt = $db->prepare("INSERT INTO etiquetas (nombre, color, orden) VALUES (?,?,?)");
        $stmt->execute([$nombre, $normColor($b['color'] ?? ''), $ord]);
        $eid = (int)$db->lastInsertId();
        audit($db, $user['id'], 'CREATE', 'etiquetas', "Creó etiqueta #{$eid}: {$nombre}", $eid);
    ...
    if ($method === 'PUT' && $id) {
        ...
        $vals[] = $id;
        $db->prepare("UPDATE etiquetas SET " . implode(', ', $sets) . " WHERE id = ?")->execute($vals);
        audit($db, $user['id'], 'UPDATE', 'etiquetas', "Editó etiqueta #{$id}", $id);
    ...
    if ($method === 'DELETE' && $id) {
        ...
        audit($db, $user['id'], 'DELETE', 'etiquetas', "Eliminó etiqueta #{$id}", $id);
        echo json_encode(['ok' => true]);
        exit;
    }
    ...
    if (function_exists('logError')) {
        logError('Error en etiquetas.php', ['message' => $e->getMessage(), 'line' => $e->getLine()]);
    }
    errSafe($e, 500);
}
